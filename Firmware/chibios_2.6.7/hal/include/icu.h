 ...
 * @param[in] channel   ICU channel that fired the interrupt
 */
typedef void (*icucallback_t)(ICUDriver *icup, icuchannel_t channel);
 ...
 * @special
 */
#define icuGetWidth(icup, channel) icu_lld_get_width((icup), (channel))
 ...
 */
/**
 * @brief   Common ISR code, ICU PWM width event.
 *
 * @param[in] icup      pointer to the @p ICUDriver object
 * @param[in] channel   ICU channel that fired the interrupt
 *
 * @notapi
 */
#define _icu_isr_invoke_pwm_width_cb(icup, channel) {                       \
  if ((icup)->state != ICU_WAITING) {                                       \
    (icup)->state = ICU_IDLE;                                               \
    (icup)->config->channels[(channel)].width_cb((icup), (channel));        \
  }                                                                         \
}

/**
 * @brief   Common ISR code, ICU Pulse width event.
 ...
 * @notapi
 */
#define _icu_isr_invoke_pulse_width_cb(icup, channel) {                     \
  if ((icup)->state == ICU_ACTIVE) {                                        \
    (icup)->state = ICU_READY;                                              \
    icu_lld_invert_polarity((icup), (channel));                             \
    (icup)->config->channels[(channel)].width_cb((icup), (channel));        \
  } else {                                                                  \
    (icup)->state = ICU_ACTIVE;                                             \
    (icup)->last_count[(channel)] = icu_lld_get_compare((icup), (channel)); \
    icu_lld_invert_polarity((icup), (channel));                             \
  }                                                                         \
}

/**
 * @brief   Common ISR code, ICU Edge detect event.
 *
 * @param[in] icup      pointer to the @p ICUDriver object
 * @param[in] channel   ICU channel that fired the interrupt.
 *
 * @notapi
 */
#define _icu_isr_invoke_edge_detect_cb(icup, channel) {                     \
  (icup)->state = ICU_READY;                                                \
  (icup)->config->channels[(channel)].width_cb((icup), (channel));          \
}
...
#define _icu_isr_invoke_overflow_cb(icup) {                                 \
  (icup)->config->overflow_cb((icup), 0);                                   \
}
/** @} */
