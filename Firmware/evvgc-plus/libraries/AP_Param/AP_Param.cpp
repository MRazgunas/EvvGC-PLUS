...
bool AP_Param::setup(void)
{
    ...
    // check the header
   eepromReadBlock(&hdr, 0, sizeof(hdr));
...
bool AP_Param::scan(const AP_Param::Param_header *target, uint16_t *pofs)
{
    ...
    while (ofs < EEPROM_24C02_SIZE) {
        eepromReadBlock(&phdr, ofs, sizeof(phdr));
...
bool AP_Param::save(bool force_save)
{
    ...
    if (ofs+type_size((enum ap_var_type)phdr.type)+2*sizeof(phdr) >= EEPROM_24C02_SIZE) {
        // we are out of room for saving variables
        //hal.console->println_P(PSTR("EEPROM full"));
        //TODO: Add debug message for full memory
        debugLog("L: Full EEPROM!");
        return false;
    }
...
bool AP_Param::load(void)
{
    ...
    // found it
    eepromReadBlock(ap, ofs+sizeof(phdr), type_size((enum ap_var_type)phdr.type));
    return true;
}
...
bool AP_Param::load_all(void)
{
    ...
    while (ofs < EEPROM_24C02_SIZE) {
        ...
        if (info != NULL) {
            eepromReadBlock(ptr, ofs+sizeof(phdr), type_size((enum ap_var_type)phdr.type));
        }
...
void AP_Param::convert_old_parameter(const struct ConversionInfo *info)
{
    ...
    eepromReadBlock(old_value, pofs+sizeof(header), sizeof(old_value));
    ...
    if (ptype == (ap_var_type)header.type) {
        ...
        debugLog("L: Bad conversion type");
